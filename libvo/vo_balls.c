...
static const vo_info_t info =
{
  "Balls",
  "balls",
  "Lewis Saunders, adapted from vo_gl2 by Arpad Gereoffy & Sven Goethel",
  ""
};

const LIBVO_EXTERN(balls)
...
static int initTextures(void)
{
  ...
  do {
    ...
    mp_msg (MSGT_VO, MSGL_V, "[balls] Needed texture [%dx%d] too big, trying ",
            texture_width, texture_height);
    ...
    if(texture_width < 64 || texture_height < 64) {
      mp_msg (MSGT_VO, MSGL_FATAL, "[balls] Give up .. usable texture size not available, or texture config error !\n");
      return -1;
    }
  ...
  mp_msg(MSGT_VO, MSGL_V, "[balls] Creating %dx%d textures of size %dx%d ...\n",
  ...
  mp_msg (MSGT_VO, MSGL_DBG2, "[balls] texture-usage %d*width=%d, %d*height=%d\n",
    ...
    for (x = 0; x < texnumx; x++) {
      ...
      switch (gl_bilinear) {
        case 0:
          ...
          mp_msg(MSGT_VO, MSGL_INFO, "[balls] bilinear off\n");
          break;
        case 1:
          glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
          glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
          mp_msg(MSGT_VO, MSGL_INFO, "[balls] bilinear linear\n");
          break;
        case 2:
          glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
          glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_NEAREST);
          mp_msg(MSGT_VO, MSGL_INFO, "[balls] bilinear mipmap nearest\n");
          break;
        case 3:
          glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
          glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_LINEAR);
          mp_msg(MSGT_VO, MSGL_INFO, "[balls] bilinear mipmap linear\n");
          break;
      }
    }
  }
...
static void gl_set_antialias (int val)
{
  ...
  if (gl_antialias) {
    ...
    mp_msg(MSGT_VO, MSGL_INFO, "[balls] antialiasing on\n");
  } else {
    glShadeModel (GL_FLAT);
    glDisable (GL_POLYGON_SMOOTH);
    glDisable (GL_LINE_SMOOTH);
    glDisable (GL_POINT_SMOOTH);
    mp_msg(MSGT_VO, MSGL_INFO, "[balls] antialiasing off\n");
  }
}
...
static void resize(int x,int y){
  ...
  mp_msg(MSGT_VO,MSGL_V,"[balls] Resize: %dx%d\n",x,y);
  if(aspect_scaling()) {
...
static int config_glx(uint32_t width, uint32_t height, uint32_t d_width, uint32_t d_height, uint32_t flags, char *title, uint32_t format) {
  XVisualInfo *vinfo, vinfo_buf;
    ...
    if (vinfo == NULL) {
      mp_msg(MSGT_VO, MSGL_FATAL, "[balls] no GLX support present\n");
      return -1;
    }
...
static int initGl(uint32_t d_width, uint32_t d_height)
{
  ...
  mp_msg(MSGT_VO, MSGL_V, "[balls] Using image_bpp=%d, image_bytes=%d, \n\tgl_bitmap_format=%s, gl_bitmap_type=%s, \n\trgb_size=%d (%d,%d,%d), a_sz=%d, \n\tgl_internal_format=%s\n",
...
config(uint32_t width, uint32_t height, uint32_t d_width, uint32_t d_height, uint32_t flags, char *title, uint32_t format)
{
  ...
  mp_msg(MSGT_VO, MSGL_V, "[balls] OpenGL Driver Information:\n");
  ...
  if(isGL12) {
    mp_msg(MSGT_VO, MSGL_INFO, "[balls] You have OpenGL >= 1.2 capable drivers, GOOD (16bpp and BGR is ok!)\n");
  } else {
    mp_msg(MSGT_VO, MSGL_INFO, "[balls] You have OpenGL < 1.2 drivers, BAD (16bpp and BGR may be damaged!)\n");
  }
...
static void check_events(void)
{
  ...
  while ( XPending( mDisplay ) ) {
    ...
    if( Event.type == KeyPress ) {
      ...
      // We naively assume the balls are devices 18, 19 and 20
...
{
  if (is_yuv) {
    mp_msg(MSGT_VO, MSGL_ERR, "[balls] error: draw_frame called for YV12!\n");
    return 0;
  }
...
static int preinit(const char *arg)
{
  ...
  if (subopt_parse(arg, subopts) != 0) {
    mp_msg(MSGT_VO, MSGL_FATAL,
            "\n-vo balls command line help:\n"
            "Example: mplayer -vo balls:noglfinish\n"
            "\nOptions:\n"
            "  noglfinish\n"
